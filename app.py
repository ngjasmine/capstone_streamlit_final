...
if selected == 'Get to know your Road Bike!':
    ...
    with col1:
        ...
        condition_subtext = st.selectbox("Condition Subtext", ['Never used. May come with original packaging or tag.', 'Used once or twice. As good as new.', 'Used with care. Flaws, if any, are barely noticeable.', 'Has minor flaws or defects.', 'Has obvious signs of use or defects.'])
    ...
    if st.button("Submit"):
        ...
        with col2:
            ...
            st.markdown("<h2 style='text-align: center;'>The predicted price class is:</h2>", unsafe_allow_html=True)
            st.markdown(f"<h3 style='text-align: center;'>{predictions[0]}</h3>", unsafe_allow_html=True)

            # Create a list of dictionaries
            class_data = [
                {"Class": 0, "Price Range": "$0- $600"},
                {"Class": 1, "Price Range": "$601-$1200"},
                {"Class": 2, "Price Range": "$1201-$2000"}
            ]

            # Convert list of dictionaries to DataFrame
            class_df = pd.DataFrame(class_data)

            # Convert DataFrame to HTML and center align it
            class_table = class_df.to_html(index=False)
            class_table = re.sub('table', 'table style="margin-left: auto; margin-right: auto"', class_table)
